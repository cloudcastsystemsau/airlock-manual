...
`AllowedFeatures` strings, parsed by `LicenseGrant.Parse`. AIR-248 made the
names canonical — legacy tokens stay accepted as synonyms and are normalised
to the canonical name at parse (`LicenseGrant.Canonical`), so fielded licences
keep working while the status DTO / licence page always show canonical names:

- `VDELAY=n` — video (NDI) delay channel cap (legacy synonyms `CHANNELS=n`,
  `VIDEO=n`, and `COUNT=n` from the ProfanityDelayService era). Absent →
  uncapped; demo → 1.
- `ADELAY=n` — audio delay channel cap (legacy synonym `AUDIO=n`). Absent → 0.
- `VENCODE=n` / `AENCODE=n` / `VDECODE=n` / `ADECODE=n` — encoder/decoder seat
  pools (legacy synonym `ENCODE=n` for `VENCODE`); a bare token means
  unlimited seats, absent means 0.
- Any other token is a named feature flag — `PANEL` (panel control surface),
  `SCRIPTING` (AIR-248, strictly enforced: the script dispatcher refuses every
  job — triggers, timers, manual Run — without it), `XHEAAC` (AIR-175 codec
  gate), `HLSDELAY` (AIR-165, reserved), and the AIR-124 role flags
  `PRIMARY`/`BACKUP`/`STANDALONE`. Checked via `LicenseService.HasFeature`,
  which also canonicalises (so `HasFeature("ENCODE")` == `HasFeature("VENCODE")`).
