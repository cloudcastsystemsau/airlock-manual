...
- **Monitor input / Monitor output** — click to listen in the browser (Opus
  over WebSocket, decoded with WebCodecs — or, on plain-http origins where
  browsers hide WebCodecs, a bundled WASM decoder, so monitoring works over
  http and https alike). One audible tap per channel; a green **MONITORING**
  tag and a volume slider appear while live:
...
mode · Censor · SCTE · Alarms · Feed stats**. Changes require the **admin** role
...
it (hidden until it re-raises); both are console-only and change neither SNMP nor
email/webhook routing. The active list pages with **Prev** / **Next**.
...
encoders, [§16](#16-audio-streaming-encoders)): create them on
...
TS data channel (a private data PID; [§18](#18-video-decoders)).
...
- **Source** — **Input** is an **Audio channel** (pick `ch{n} — {name}`), an
  ...
  **backend** (**asio** on Windows, **alsa** on Linux, **sim** for headless
  tests), the **device** (ASIO drivers and capture-capable ALSA PCMs are
  probed; free-text otherwise), the **input** channel pair (ASIO — named pairs
  when the driver reports them) and the capture **sample rate**. **Channels**
  is `1 (mono)` or `2 (stereo)` (*HE-AAC v2 renditions need a stereo source*)
  and doubles as the capture width for a sound-device source.
...
([§17](#17-audio-decoders-and-rtp-transport)). Beyond GPIO, the **RTP & data**
...
*Manual generated against Airlock as of 2026-07-18 (main @ 0f90312,
AIR-1…246). Screenshots were retaken on a **licensed** live instance
...
Axia LWRP device; and a video decoder cannot reach a live "Receiving" state on
the capture box (no GStreamer SRT), so §18's card shows a not-running state.*
