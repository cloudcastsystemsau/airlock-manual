...
Around both kinds sit a shared **Audio IO** registry — the machine's sound
devices declared once and referenced by name platform-wide
([§16](#16-audio-io)) — plus Axia Livewire GPIO panel integration, delayed
data routing, alerting, primary/backup redundancy, and a runtime scripting
...
2. [Getting started](#2-getting-started)
   - [Installing](#installing) · [**Third-party prerequisites**](#third-party-prerequisites-install-on-the-server) · [First sign-in](#first-sign-in) · [**The first-run wizard**](#the-first-run-wizard) · [The console at a glance](#the-console-at-a-glance)
3. [Operations dashboard and the Video console](#3-operations-dashboard-and-the-video-console)
   - [The view-only dashboard](#operations--the-view-only-dashboard) · [The video console](#video--delay-channels--the-video-console) · [**Listening: meter speakers and the global player**](#listening-meter-speakers-and-the-global-player) · [Adding, disabling and deleting channels](#adding-disabling-and-deleting-channels-admin)
...
5. [Channel configuration](#5-channel-configuration)
   - [Source & name](#source--name) · [Fill + dayparted schedule](#fill--static-assignment-and-dayparted-schedule) · [Delay mode](#delay-mode) · [**Aux audio — incoming channels & extra tracks**](#aux-audio--incoming-channel-policies-and-extra-tracks) · [Censor parameters](#censor-parameters) · [Feed stats](#feed-stats) · [Audio processing](#channel-audio-processing-trim--eq--compressor)
...
8. [Video encoders (SRT/SCTE-35)](#8-video-encoders-srtscte-35)
...
16. [Audio IO](#16-audio-io)
    - [Inputs and outputs](#inputs-and-outputs) · [Declaring an object](#declaring-an-object) · [Status, metering and in-use-by](#status-metering-and-in-use-by) · [Who references IO objects](#who-references-io-objects)
17. [Audio streaming encoders](#17-audio-streaming-encoders)
...
18. [Audio decoders and RTP transport](#18-audio-decoders-and-rtp-transport)
...
19. [Video decoders](#19-video-decoders)
    - [The Video Decoders page](#the-video-decoders-page) · [The video decoder editor](#the-video-decoder-editor)
20. [Roles and permissions](#20-roles-and-permissions)
21. [Alarms and troubleshooting](#21-alarms-and-troubleshooting)
...
- For **video channels**: the **NDI runtime** on the server (without it video
  ...
  the Fills page warn when missing). Install steps for both are in
  [Third-party prerequisites](#third-party-prerequisites-install-on-the-server) below.
...
unless `--DataDir` says otherwise. See
[Third-party prerequisites](#third-party-prerequisites-install-on-the-server)
for the media components to install alongside it.
...
### Third-party prerequisites (install on the server)

The installers on both platforms are **self-contained**: the Windows MSI and
the Linux tarball each carry the **.NET 8 runtime** (install .NET 8 yourself
only when running from source — `winget install Microsoft.DotNet.SDK.8` /
`sudo apt install dotnet-sdk-8.0`) and the audio-encoder codec libraries
**FDK-AAC** and **xHE-AAC** (in the `natives` folder). **LAME** (the MP3
codec for Icecast audio streaming) is likewise bundled on Windows; on Linux
install the system library — `sudo apt install libmp3lame0`. Three media
components are **not** bundled — for licensing and size reasons — and must be
installed on the server for the features that use them:

| Component | Needed for | Windows | Linux |
|---|---|---|---|
| **NDI Runtime** — version 6 (match the tested **6.3.2.0**) | All **video NDI** I/O: receiving sources and sending the delayed output. Without it, video channels run in **ENGINE-SIM** (a synthetic demo source — no real NDI). | [ndi.video/tools](https://ndi.video/tools/) (NDI Core Suite / Runtime); direct installer `https://downloads.ndi.tv/SDK/NDI_SDK/NDI 6 Runtime.exe`; or `winget install --exact --id NDI.NDIRuntime` | `libndi.so.6` (from the NDI SDK for Linux or a vendor runtime bundle) on the loader path: `sudo cp libndi.so.6* /usr/local/lib && sudo ldconfig` |
| **ffmpeg** (includes **ffprobe**) | **Fill assets** — probing on upload and conform-on-assign — and **dump-clip / fill browser previews**. Put both binaries on `PATH`, or point `FFMPEG_PATH` / `FFPROBE_PATH` at them (Windows also probes `C:\ffmpeg\bin`). | [ffmpeg.org/download.html](https://ffmpeg.org/download.html) → Windows builds: [gyan.dev](https://www.gyan.dev/ffmpeg/builds/) or [BtbN](https://github.com/BtbN/FFmpeg-Builds/releases) | `sudo apt install ffmpeg` |
| **GStreamer** — 1.20+, runtime **+ plugins** | The **video encode** option (SRT / UDP-TS contribution, H.264 / H.265) and **video decoders** (SRT/TS → NDI). Install the **complete** package so the **SRT** (`srtsrc` / `srtsink`) and H.264/H.265 codec plugins are present. | [gstreamer.freedesktop.org/download](https://gstreamer.freedesktop.org/download/) — MSVC x86_64 runtime installer, **Complete** feature set (Airlock finds it via the `GSTREAMER_1_0_ROOT_MSVC_X86_64` variable the installer sets) | `sudo apt install libgstreamer1.0-0 gstreamer1.0-plugins-base gstreamer1.0-plugins-good gstreamer1.0-plugins-bad gstreamer1.0-plugins-ugly gstreamer1.0-libav` (`plugins-bad` carries the TS mux, SRT and OpenH264 elements; `x264enc` lives in `plugins-ugly`) |

For the **NVENC** hardware-encoder preset the server additionally needs an
**NVIDIA GPU with a current driver** (GStreamer's `nvcodec` plugin); otherwise
choose the software **x264** / **OpenH264** presets. After installing any of
these, **restart the Airlock service** — the footer pills **NDI: runtime
active** and **Media tooling: ready** confirm they were detected. None of this
is needed for audio-only or plain NDI-passthrough use; each component gates only
its own feature.
...
On a fresh install, the first admin sign-in lands in a two-step **first-time
setup** wizard instead of the console:

![First-run wizard](img/72-first-run-wizard.png)

1. **Password** — replace the bootstrap administrator password before going
   on air (when you have just typed the bootstrap password to log in, the
   wizard reuses it as the current password).
2. **NDI network** — pick the network adapters NDI should use (one shared
   allow-list for receive *and* send; nothing selected = every adapter,
   including WiFi). Selecting adapters offers **Finish & restart Airlock**,
   since the allow-list applies at process start
   ([§13.10](#1310-ndi-adapters-server--ndi)).

**Set up later** skips the wizard for the current session only — it returns at
every login until completed once. Servers upgraded from an earlier Airlock are
treated as already set up and never see it. An admin can re-run it at any time
(nothing is wiped) with **Re-run setup wizard** in Server → General
([§13.1](#131-general-settings-server--general)).

### The console at a glance

- **Top navigation**:
  - *Operations* — the view-only dashboard over everything ([§3](#3-operations-dashboard-and-the-video-console));
  - *Video* — **Delay channels** (the video console), **Encoders**,
    **Decoders**, **Fills**, **Dump clips**;
  - *Audio* — **IO** (the declared sound-device endpoints, [§16](#16-audio-io)),
    **Delay channels**, **Encoders**, **Decoders**, **Fills & schedules**,
    **Censor schedules**, **Dump clips**;
  - *Data Receivers* — TCP Servers, TCP Clients, UDP Servers, Routing,
    Axia GPIO;
  - *Scripting* (admin);
  - *Server* (admin) — **General**, **NDI**, **Email (SMTP)**,
    **Authentication**, **Users**, **Alert groups**, **Audit log**,
    **License**, **Redundancy**, **API access**, **Panel control**;
...
channel: state badge, source line, input/output previews, meters — including
a bar per detected incoming stereo pair beyond the programme pair — compact
...
audio channel with its depth bar and meters), and — when any are configured —
**Audio encoders** and **Audio decoders** bands (one tile each with endpoints,
live stats and metering, [§17](#17-audio-streaming-encoders) /
[§18](#18-audio-decoders-and-rtp-transport)). Clicking any tile jumps to the
...
- **Header** — channel name; engine badge (`NDI delay engine`, or `engine-sim`
  ...
  amber **UNLICENSED** button — see [§14](#14-licensing)); the **state badge**;
  the **sliders icon** that opens audio processing
  ([§5](#5-channel-configuration)); and the gear that opens channel settings.
  (Contribution encoders live on their own page — [§8](#8-video-encoders-srtscte-35).)
...
- **Audio meters** — L/R RMS bars with peak-hold, −60…0 dBFS, turning amber at
  −9 and red at −3, for the programme input and output. When the source
  carries **more than one stereo pair**, a compact **input | output** meter
  row appears per extra detected pair — the same per-pair view as the
  channel's Aux audio tab ([§5](#5-channel-configuration)) — and aux audio
  tracks meter beneath them. Every meter ends in a small **speaker icon** —
  see [Listening](#listening-meter-speakers-and-the-global-player) below.

  ![Per-pair meters and speakers](img/08-audio-monitoring.png)

- **Counters** — `Depth` (frames and seconds), `In`/`Out` frame counts,
  `Drops` (red when non-zero), `Holds` (hold-last-frame repeats), `SCTE`
  (splices inserted) — with a small **reset** control beside the cumulative
  counters to zero them without restarting anything.
- **Alarm strip** — active alarms in red, e.g. `ALARM_SOURCE_LOST`
  ([§21](#21-alarms-and-troubleshooting)).
- **Transport buttons** — Build / Roll out / Dump / Trigger ad break
  ([§4](#4-operating-the-delay)).

### Listening: meter speakers and the global player

Confidence monitoring is per meter: **every audio meter in the console ends in
a small speaker icon** — programme input/output, each extra incoming pair, each
aux track, encoder and decoder taps, and declared Audio IO inputs. Click a
speaker to listen to exactly that signal in the browser (Opus over WebSocket,
decoded with WebCodecs — or, on plain-http origins where browsers hide
WebCodecs, a bundled WASM decoder, so monitoring works over http and https
alike). Because the delay taps are **per stereo pair**, listening to pair 3
plays pair 3 — not a downmix, and not the first pair.

**One tap is audible console-wide.** Playback belongs to a **global player
docked above the footer** — it names the tap it is playing, shows its status,
and carries a volume slider (persisted per browser) and a **Stop** button.
It keeps playing while you navigate between pages; clicking another speaker
takes over the player. Signing out stops playback, and a dropped socket shows
as *connection lost* rather than a stale playing state. Listening is
view-only, so every role gets it.

![The global audio player](img/73-footer-player.png)
...
When a channel is passing SCTE cues from its source ([§8](#8-video-encoders-srtscte-35)),
...
- **Slate** — cover the output with the technical-difficulties still
  ...
  stages, so **both the NDI output and the encoder carry it**. Slate is
  engaged from GPIO, control panels or the API; while it is covering the
  output a **Slate ●** button (pulsing red) appears on the card — click it to
  release. It holds until released (no auto-timeout) and works in any state.
...
The gear on a channel card opens a **full-page tabbed editor** — headed
**Video delay channel — {name}** with a **← Video delay channels** back link
(no pop-up dialog: you leave via the back link, saving with **Save config** /
**Discard changes** as you go). Its tabs are **Source & name · Fill · Delay
mode · Aux audio · Censor · SCTE · Alarms · Feed stats**. Changes require the **admin** role
(Feed stats is visible to everyone); source, fill and delay-mode changes are
only permitted while the channel is **Live**. Encoders, decoders and audio
channels all open the same way — a card's gear opens its own full-page editor,
and the **+ Add channel / + Add encoder / + Add decoder** controls expand to a
name field (Enter creates and opens the new editor, Esc cancels).
...
### Aux audio — incoming channel policies and extra tracks

Multi-track NDI sources (say 8 channels as 4 stereo pairs — programme, second
language, clean/international sound, commentary) and locally-added audio both
live on the **Aux audio** tab. Its two halves:

![Aux audio tab — incoming policies and extra channels](img/74-aux-audio-tab.png)

**Incoming audio** is the channels detected on the NDI source itself. The
header reads **"N channels detected"** (from the live feed; while the source
is offline the last-seen count is kept, flagged *last seen, source offline*).
One row per detected stereo pair carries a **policy** dropdown:

- **Protected — censor + processing apply** *(default)* — the pair rides the
  delay and every protection: censor/bleep, dump fill, slate mute and the
  channel's audio processing.
- **Original — pass through untouched** — delayed in time with everything
  else, but the content is never altered: no censor, no processing, no slate
  mute. Note the delay timeline itself is global — a **DUMP still collapses
  time for every pair** (they all ride one buffer); only the content is
  untouched.
- **Replaced — swap in a defined track** — the pair's channels are overwritten
  with a locally captured source: pick one of the aux tracks defined under
  *Extra channels* below (it must match the pair's width). The replacement is
  delayed through the same ring, so it stays time-aligned with the video.

Each row shows live **input** (as received) and **output** (as aired — policy,
censor and processing applied) meters side by side, each with its own
speaker — so you can watch, and hear, a censor or a replacement take effect.
The same per-pair meters appear on the channel card and the dashboard tile.

**Extra channels** are aux tracks appended to the delayed NDI output after the
incoming audio (or consumed by a *Replaced* pair above). Each track has a
name, a **source** — another **NDI stream** (audio-only receive) or a channel
slice of a **declared Audio IO input** ([§16](#16-audio-io); with ASIO the
named-channel pairs are offered) — and a **Fill audio** policy for what plays
during Build and other fill states: **Parent** (the main fill's audio) or an
**own uploaded audio file** (conformed and looped to the fill's period).
Tracks are captured live and delayed in lockstep with the video. **Add track**
appends one; **Save aux tracks (restarts engine)** applies.

Changing the channel's **NDI source clears the incoming-pair policies** (they
are tied to the old source's audio layout — the console warns before an actual
change); aux track definitions are kept.
...
- **Slate (technical difficulties)** — a still held at the channel **output**
  ...
  programme audio while slated**. It is engaged from GPIO, control panels or
...
- **Cue output** — **Re-insert cues on the NDI output** (*"the delayed
  ...
  needs the encoder, [§8](#8-video-encoders-srtscte-35)), and **Pass other
...
**Video → Dump clips** lists the video channels' clips, and **Audio → Dump
clips** the audio channels' WAV dumps — the same auto-refreshing table split
by kind: when, channel, duration, who/via, and status — `writing` (violet)
while the file is being finalised, `ready` (green) once playable as MP4,
`raw` (amber) for audio-channel WAV dumps.
...
Active alarms refresh every few seconds. Operators can **Ack** an active alarm
(it stays listed, dimmed, with an **ACK** chip naming who and when) or **Dismiss**
it — a dismissed alarm disappears from **every display surface** (the Alarms
page, channel cards and dashboard tiles, encoder/decoder cards, and hardware
panel lamps fed from display state) until it re-raises. Both are display-only:
SNMP, alarm emails, webhooks and GPO alarm lamps are never silenced from the
console — plant monitoring always sees the raw state. The active list pages
with **Prev** / **Next**.
...
1. **On the group** (Server → Alert groups) — tick the alarm categories the
   group cares about (*Video delay*, *Encoding*, *Audio delay*, *System*, plus
   *Audio encoding*, *Audio decoding* and *Video decoding*) and
...
answer for their own state. The MIB module is downloadable from the console —
**Server → General**, the **download the MIB** link (saves `AIRLOCK-MIB.mib` for
your NMS) — and a **bind** selector restricts the agent to a single adapter's
address (default: every adapter).
...
Video encoders are **standalone entities** (like the audio streaming
encoders, [§17](#17-audio-streaming-encoders)): create them on
**Video → Encoders** with **+ Add encoder**, each with its own name, enable
state and licence seat. An encoder sources its feed from any **delay
channel's aired output** or an **external NDI source** — and **several
encoders can source the same channel** (e.g. one SRT and one UDP contribution
of the same protected feed). The **cog** on an encoder card opens its editor:

![Encode modal](img/13-encode-modal.png)

- **Input source** — *"Delay channel output (PGM)"* (pick the channel; the
  encoder carries its **as-aired** output — delay, censor masks, slate and
  seam fades included) or *"External NDI source"*: any NDI source encoded
  directly, **bypassing the delay**. Raster and framerate lock from the
  source's first frame (the encoder restarts once when they differ from the
  last known format).
...
Each encoder runs as a **supervised child process** — an encoder crash never
affects the NDI programme output; the supervisor restarts it and raises/clears
`ALARM_ENCODE_DOWN` (an encoder-scoped alarm row on the Alarms page, like the
audio encoders'). On a redundancy **backup with controls locked**, encoder
pipelines run but their **outputs are suppressed** — no SRT listener opens and
no UDP datagrams leave the box, so a master+backup pair never double-feeds the
plant (an *output suppressed* badge shows on the card).

**Video → Encoders** shows one card per encoder with a live output preview
(the watermark is visible on unlicensed feeds), its source and endpoint,
loudness/A-V/drops/SCTE stats, its **licence seat** state with Assign/Release
buttons, and Enable/Disable/Delete (admin). The Operations dashboard mirrors
enabled encoders as read-only tiles.

Instances upgraded from earlier versions migrate automatically: each channel
that had the per-channel encode branch becomes one standalone encoder named
"*channel* encoder", sourcing that channel, with its enable state and seat
preserved.
...
> to a local device; see [§17](#17-audio-streaming-encoders) and
> [§18](#18-audio-decoders-and-rtp-transport).
...
The **cog** on the card opens the channel's **full-page editor** (headed
**Audio delay channel — {name}**, tabs **Device · Delay & build · Dump · Censor
· Alarms**; return via **← Audio delay channels**). Admins can **rename** the
channel on the Device tab. Configuration changes
...
- **Device** — backend `asio` (Windows; e.g. the Axia IP-Audio driver),
  `alsa` (Linux), `sim` (no hardware — demo/test) or **`io (declared)`** —
  reference the machine's declared **Audio IO objects** ([§16](#16-audio-io))
  instead of raw device fields: pick an **Input (IO object)** and an
  **Output (IO object)**. The two must live on the **same sound card** (the
  output picker only offers matching objects) because the delay opens them as
  one clock-linked duplex device — that drift-free pairing is unchanged, just
  expressed through the registry (the IO page then shows the delay holding
  both objects, with the input's live meter and listen tap). With ASIO, a
  **Device** dropdown lists the installed drivers and **Main input / Main
  output** offer the driver's *named channel pairs* (Axia Livewire source
  names collapse to one entry, e.g. `1–2 · Livewire 1`); alsa/sim fall back to
  numeric channel offsets. Optional **post-censor offset** (an extra-delayed
  clean output on the pair above the main pair) and **Rollout return channel**
  (the mix-minus return pair — see below).
...
over TCP, GPIO and scripting, and audio DUMPs land in **Audio → Dump clips**
as WAV.
...
optional on-connect and keep-alive messages — `\n`, `\r`, `\t` in these are
sent as control characters; use `\\` for a literal backslash), **UDP
Server**. Each has raw or
...
audio encoder's RTP data channel, [§17](#17-audio-streaming-encoders)), or back
...
([§18](#18-audio-decoders-and-rtp-transport)): add an **RTP decoder feed** bound
to a decoder, and far-end messages released in sync with the audio behave like
any received data — firing `dataReceived`, routing and logging. A sibling
**Video Decoders** tab does the same for data carried on a **video** decoder's
TS data channel (a private data PID; [§19](#19-video-decoders)).
...
Running/Down or firing a cue, [§17](#17-audio-streaming-encoders)),
`videoDecoderEvent` (a video decoder going Running/Down/StreamLost,
[§19](#19-video-decoders)), `gpi` (a concrete LWRP device/port/pin and edge,
...
— and channel; [§8](#8-video-encoders-srtscte-35)), `scriptDelayed` (a
...
Below the settings sit three admin controls:

- **Restart service** — gracefully restart Airlock from the console (with a
  force-exit failsafe; the service manager brings it back). This is how
  restart-to-apply settings — NDI adapters
  ([§13.10](#1310-ndi-adapters-server--ndi)), listener bind addresses — take
  effect without shell access to the box. The channels' delay buffers do not
  survive a restart, so time it off air.
- **Re-run setup wizard** — re-arm the first-run wizard
  ([§2](#2-getting-started)) without wiping anything; it relaunches
  immediately for the current admin and returns at each admin login until
  completed.
- **Factory reset** (red zone) — a **selective** wipe: tick what to reset
  (channels incl. fill media and trigger templates · data receivers & routes ·
  scripts · encoders & decoders · GPIO · alert groups · users, API clients &
  panel tokens · server settings incl. redundancy · licence local activation),
  type **RESET** to confirm, and Airlock wipes the selected categories and
  restarts. **Audit history and dump clips are always kept** (compliance
  records). Wiping users re-seeds the bootstrap admin and re-arms the
  first-run wizard.
...
listing control actions (**Time · Channel · Action · Source · Detail**), newest
first, **50 per page** — page with **Newer** / **Older**, and admins can **Purge
log** (which also clears the stored alarm history — confirmed, and itself
audited). The full history is available via `GET /api/audit?skip&take`. A
checkpointed copy of the configuration database can be downloaded by admins from
...
### 13.10 NDI adapters (Server → NDI)

By default the NDI runtime uses **every network adapter** — including WiFi,
where reception is typically terrible. **Server → NDI** pins NDI traffic to
selected adapters:

![NDI adapter allow-list](img/75-server-ndi.png)

- The panel lists the machine's adapters with a toggle each. The selection is
  **one shared allow-list for receive and send** (the runtime has no
  per-direction split); nothing selected = every adapter.
- The setting is **per-instance**: Airlock writes its own `ndi-config.v1.json`
  into a private directory, so other NDI applications on the box are
  untouched.
- The allow-list applies when the NDI runtime loads — i.e. at **process
  start**. The panel shows a restart-required notice and offers **Save &
  restart**; the first-run wizard ([§2](#2-getting-started)) sets the same
  rows on a fresh install.
- The adapter list is machine-local and is **not replicated** to a redundancy
  backup ([§13.7](#137-redundancy-primarybackup)) — each node keeps its own.
...
**audio streaming-encoder seats** (`AENCODE=n`, [§17](#17-audio-streaming-encoders)),
**audio decoder seats** (`ADECODE=n`, [§18](#18-audio-decoders-and-rtp-transport))
and **video decoder seats** (`VDECODE=n`, [§19](#19-video-decoders))
...
HLS renditions ([§17](#17-audio-streaming-encoders)). Licences issued before
...
management/automation LAN (there is an allow-list; default localhost only). A
**bind** selector (Server → General) can pin the listener to one adapter's
address — it applies at the next server restart, with the allow-list on top; the
**Panel control** listener ([§13.9](#139-panel-control-server--panel-control))
and the SNMP agent ([§6.2](#62-snmp-monitoring--control)) have the same
bind-address option.
...
**Audio → IO** is the machine's audio patch bay: declare each sound-device
endpoint **once** — a name, a backend, a device, a channel range — and
everything else on the platform references it by name. One physical device is
opened once, however many features use it; renaming a cable never means
re-configuring five pages.

![Audio IO page](img/76-audio-io.png)

### Inputs and outputs

The page has two groups, and the model differs deliberately:

- **Inputs are shared.** A declared input is one physical capture open whose
  signal **fans out** — any number of consumers can take channel slices of the
  same open at once: aux audio tracks ([§5](#5-channel-configuration)), audio
  streaming encoders ([§17](#17-audio-streaming-encoders)), an audio delay's
  capture side ([§9](#9-audio-profanity-delay)).
- **Outputs are sinks**, assigned to **exactly one renderer** (an audio
  delay's playout). Two writers on one output is a hardware impossibility,
  not a policy — the registry refuses a second assignment.

An audio delay channel pairs **one input and one output on the same sound
card**, which the runtime opens as its usual clock-linked duplex device
([§9](#9-audio-profanity-delay)).

### Declaring an object

**+ Add input** / **+ Add output** (admin) opens the inline editor:

- **Name** — how every other page will refer to it (max 24 chars).
- **Backend** — `alsa`, `asio` or `sim`, with probe-backed device pickers:
  installed **ASIO drivers** on Windows, capture/playback **ALSA PCMs** on
  Linux, and a free-text field as fallback (e.g. `hw:1,0`).
- **Width** and **First channel** — the channel slice of the physical device
  this object covers (with ASIO the first-channel picker offers the driver's
  *named* channels, e.g. Livewire pair names); **Rate** — 44.1/48/96 kHz.

Editing an object that is **in use** warns that device changes apply when its
consumers next (re)start. **Delete is blocked while the object is referenced
or claimed** — the error names the blocking claims, so nothing quietly loses
its audio.

### Status, metering and in-use-by

Each row carries a status lamp — grey **idle** (declared, nothing using it),
amber **claimed, no data**, green **receiving**, or red with the device's last
error (a failed open also raises the server-scoped `ALARM_AUDIO_IO_LOST`,
[§21](#21-alarms-and-troubleshooting)) — and an **"in use by"** line naming
each consumer and the channels it holds (e.g. `ch3 aux "commentary"
(ch 1–2)`).

While any consumer holds an input, the row shows a **live level meter**
labelled with the consumer it arrives through, and — when that consumer
publishes a listen tap — the standard **speaker** icon feeding the global
player ([§3](#listening-meter-speakers-and-the-global-player)). Idle inputs
show no meter (nothing holds the device, and the registry won't open it just
to look); **output** objects are not metered yet — their signal belongs to
the assigned renderer.

### Who references IO objects

- **Aux audio tracks** on a video delay channel take a channel slice of an
  input ([§5](#5-channel-configuration)).
- **Audio streaming encoders** with a device source pick an **IO input**
  instead of raw device fields ([§17](#17-audio-streaming-encoders)).
- **Audio delay channels** with backend `io (declared)` pair an input and an
  output ([§9](#9-audio-profanity-delay)).

Legacy configurations migrate automatically on upgrade: each distinct device
tuple that aux tracks or encoders used becomes a declared object, and the
references are rewritten — no manual steps.

On a **redundancy pair**, IO objects replicate to the backup like other
configuration — including, today, the device fields. When the backup box has
different hardware, declare objects with device names that resolve on both
machines where possible (per-node device overrides are planned).

## 17. Audio streaming encoders
...
is a distinct subsystem from the video **Encode option** ([§8](#8-video-encoders-srtscte-35)):
...
- **Source** — **Input** is an **Audio channel** (pick `ch{n} — {name}`), an
  **NDI source** (pick from the finder), or a **Sound device** — capture
  straight off a sound card with no delay channel in between. The device case
  references the registry: pick a declared **IO input** ([§16](#16-audio-io))
  and the **channel offset** within that object (ASIO named pairs are shown
  when the driver reports them). Upgrades rewrite older raw-device encoder
  configs into declared objects automatically. **Channels** is `1 (mono)` or
  `2 (stereo)` (*HE-AAC v2 renditions need a stereo source*) and doubles as
  the capture width for a sound-device source.
...
([§18](#18-audio-decoders-and-rtp-transport)). Beyond GPIO, the **RTP & data**
tab's **Data channel inputs** fieldset accepts local **UDP/TCP servers** (one per
slot) whose messages (≤200 bytes) ride the channel directly — an alternative to
feeding data in through a receiver route.

![Encoder GPIO/data carriage](img/64-encoder-data-carriage.png)

## 18. Audio decoders and RTP transport

The receive side of RTP audio. A **decoder** listens for an RTP stream — from
an Airlock encoder's RTP send ([§17](#17-audio-streaming-encoders)) or any
...
- **Buffering** — **Jitter buffer** (ms) and **stream lost after** (ms). With
  ...
  (L × D × ptime).
- **Playout** — **Backend** (`sim` / `alsa` / `asio`), the output **device**
  (probed pickers: installed ASIO drivers, or playback-capable **ALSA PCMs**
  on Linux — free-text as fallback) and channel, **Device rate** and
  **buffer** (frames). An **Audio processing** strip runs live on the decoded
  output ahead of the device.
...
the audio ([§17](#17-audio-streaming-encoders)). Tick **GPIO/data from the
sender (+6)** in the decoder's **Data channel** row, and a **GPO outputs (carried
GPIO)** table appears: map each incoming **slot** to a local LWRP target either
**per pin** (source **pin** → **device / GPO / pin**) or as a **whole-port
passthrough** — the slot's five pins drive a local GPO **port** straight
through in one row. Both forms take a **level** follow or a **pulse** (with a
...
the plant. Released **data messages** can also fan out straight from the decoder:
a **Data channel outputs** fieldset (on the decoder's **Data & GPO** tab)
forwards each message to **UDP endpoints, a TCP client, or a TCP-server
broadcast** as it plays out (silent on a locked backup).

![Decoder GPO release of carried GPIO](img/65-decoder-gpo-carriage.png)

## 19. Video decoders

Where the encode option ([§8](#8-video-encoders-srtscte-35)) *sends* a
...
- **Transport** — **SRT** or **UDP TS** (*"plain MPEG-TS — unicast, multicast,
  ...
  **key** length (**AES-128 / 192 / 256**). Match the encoder's **FEC (packet
  filter)** here when one is set ([§8](#8-video-encoders-srtscte-35)).
...
| `ALARM_AUDIO_IO_LOST` | A declared Audio IO object's device failed to open | Server-scoped. The failing object shows a red lamp and its last error on **Audio → IO** ([§16](#16-audio-io)) — check the device name and that the hardware/driver is present. |
...
| `ALARM_AENCODE_DOWN` | Audio streaming-encoder child not running | Supervisor restarts it; check the source and outputs ([§17](#17-audio-streaming-encoders)). |
...
| `ALARM_ADECODE_DOWN` | Audio decoder child not running | Supervisor restarts it; check the playout device ([§18](#18-audio-decoders-and-rtp-transport)). |
...
| `ALARM_VDECODE_DOWN` | Video decoder child not running | Supervisor restarts it; check the input transport and the decoder ([§19](#19-video-decoders)). |
...
*Manual generated against Airlock as of 2026-07-21 (main @ 88f44bc,
AIR-1…282). Screenshots were retaken on a **licensed** live instance
(serial-activated) using the built-in colour-bars + tone test source, against
the current full-page tabbed settings editors. Exceptions: §14's channel shot is
deliberately unlicensed to show the watermark; the Stream Deck plugin shots
(§13.9) are from the Elgato Stream Deck app; the GPIO pages (§10) need a live
Axia LWRP device; and the capture rig leaves encoders/decoders disabled (no
live sender to receive), so §19's card shows a not-running state.*
